...
export default class App extends Component {
    ...
    render() {
        ...
        <Swiper style={styles.wrapper} showsButtons={false} >
            ...
            <View style={styles.container}>
                <Button title = 'ACTIVATE GPS' onPress = {this.onButtonClickHandler}> </Button>
              
                    
                {this.state.showCoords ? <Text>LON: {this.state.longitude} </Text> :null}
                {this.state.showCoords ? <Text>LAT: {this.state.latitude} </Text> :null}                    
...
const styles = StyleSheet.create({
    ...
    container: {
       flex: 1,
       backgroundColor: '#001F5B', alignItems: 'center', justifyContent: 'center' 
       },
    photo: {
        width: 200, height:200,
    },
    namePlate:{
        padding: 10, fontSize: 28,position:'absolute', bottom: 30
    }
   });